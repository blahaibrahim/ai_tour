...
| Data | Static `allLocations` list of 8 hardcoded `Location`s | Becomes a seeded database table; models need `fromJson`/`toJson` |
...
└──────────────────────────────┬────────────────────────────┘
                               │ shared secret / proxy auth
                               ▼
┌───────────────────────────────────────────────────────────┐
│  Modal — Hunyuan3D 2.1 on an L4 GPU                       │
│    (never reachable directly from the app)                │
└───────────────────────────────────────────────────────────┘
```
...
**Phase 3 — the 3D feature**
Harden the Modal endpoint · Edge Function proxy · job table · camera flow ·
real `.glb` viewer replacing the photo cube. This is the headline feature and
depends on both prior phases.

**Phase 4 — polish**
i18n · dark mode · real LLM behind the AI panels.

Phases 1–2 are unglamorous but everything else sits on them. Doing phase 3
first means building the job queue twice.
...
- **Maps** — CartoDB basemaps and Nominatim are free but have usage policies
  that require attribution and rate limiting. Already in use; see
