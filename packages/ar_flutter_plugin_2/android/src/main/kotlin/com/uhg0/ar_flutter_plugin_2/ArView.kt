...
class ArView(
    ...
    private val onObjectMethodCall =
        MethodChannel.MethodCallHandler { call, result ->
            when (call.method) {
                ...
                "setMetallic" -> handleSetMetallic(call, result)
                else -> result.notImplemented()
            }
        }
    ...
    private fun handlePlayAnimation(call: MethodCall, result: MethodChannel.Result) {
        try {
            val name = call.argument<String>("name")
            val animationName = call.argument<String>("animation")
            val loop = call.argument<Boolean>("loop") ?: true
            if (name == null || animationName == null) {
                result.error("INVALID_ARGUMENT", "Node name and animation are required", null)
                return
            }
            nodesMap[name]?.let { node ->
                node.playAnimation(animationName, loop = loop)
                result.success(true)
            } ?: result.error("NODE_NOT_FOUND", "Node with name $name not found", null)
        } catch (e: Exception) {
    ...
    private fun handleSetMetallic(call: MethodCall, result: MethodChannel.Result) {
        try {
            val name = call.argument<String>("name")
            val metallic = call.argument<Double>("metallic")?.toFloat()
            if (name == null || metallic == null) {
                result.error("INVALID_ARGUMENT", "Node name and metallic value are required", null)
                return
            }
            nodesMap[name]?.let { node ->
                node.modelInstance?.materialInstances?.forEach {
                    it.setParameter("metallic", metallic)
                }
                result.success(true)
            } ?: result.error("NODE_NOT_FOUND", "Node with name $name not found", null)
        } catch (e: Exception) {
            result.error("METALLIC_ERROR", e.message, null)
        }
    }
