...
from ingestion import overpass, photos, scoring, wikidata, wikipedia
...
def _ingest_one_tile(client, tile_id: str) -> None:
    ...
    def process_poi(poi):
        ...
        if summary and summary.get("extract"):
            ...
            # Generate deterministic semantic description from OSM tags
            desc_parts = []
            t = poi["tags"]
            if t.get("natural"):
            ...
            if t.get("access") and t.get("access") != "yes":
                extras.append(f"({t['access']} access)")
        ...
        # LLM rewrite pass: only for Wikipedia-sourced blurbs, not fallback strings.
        # Degrades silently — falls back to the truncated raw on any failure.
        if summary and summary.get("extract"):
