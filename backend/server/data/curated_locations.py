...
role: the hardcoded fallback `locations_repo.py` degrades to when Supabase is
...
the caller's position, computed via `haversine_km`, not a fact about the
place.
"""
...
_LOCATIONS_BY_ID = {loc["id"]: loc for loc in CURATED_LOCATIONS}
...
def locations_within_radius(lat: float, lng: float, radius_km: float, limit: int | None = None) -> list[dict]:
    """Curated locations within `radius_km`, each annotated with `distance_km`.
    ...
    for loc in CURATED_LOCATIONS:
        ...
        annotated.append({**loc, "distance_km": round(d, 1)})

    annotated.sort(key=lambda l: l["distance_km"])
    within = [l for l in annotated if l["distance_km"] <= radius_km]
    result = within if within else annotated[:5]
